(function () {
    ...
    const DELETE_MSG = (window.i18n && window.i18n.dashboard && window.i18n.dashboard.deleteConfirm)
        || 'Are you sure you want to delete this file? This action cannot be undone.';

    // ── Search ────────────────────────────────────────────────────────────
    document.getElementById('adminSearchForm')?.addEventListener('submit', function (e) {
        e.preventDefault();
        QD.loadDynamic('/admin/dashboard?' + new URLSearchParams(new FormData(this)), 'fileDynamicContent');
    });

    // ── Delegated events on the stable wrapper ────────────────────────────
    ...
    wrapper.addEventListener('change', e => {
        ...
        if (el.closest('[data-perpage-form]')) {
            QD.loadDynamic('/admin/dashboard?' + new URLSearchParams(new FormData(el.closest('form'))), 'fileDynamicContent');
            return;
        }
    ...
    function refreshCardStyle(card) {
        ...
        // Both active → red; only keep → teal; only hidden → rose; default → primary
    ...
    // ── Toggle: optimistic update + background POST ───────────────────────
    ...
    // ── Delete: animate out + POST + remove card ──────────────────────────
