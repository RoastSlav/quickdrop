...
public class FileUtils {
    ...
    /** Formats bytes with two decimals, e.g. {@code "1.50 MB"}. */
    ...
    /** @return absolute download URL, e.g. {@code https://example.com/file/abc-123} */
    ...
    /**
     * Prefers {@code X-Forwarded-Proto}/{@code X-Forwarded-Host} so links are correct
    ...
    /**
     * If the scheme was forwarded but neither Host nor Port were, request.getServerPort() is
     * the backend's own raw listening port (e.g. 8080) -- pairing that with a forwarded
     * "https" would leak it as "https://host:8080". X-Forwarded-Port is honored if sent;
     * otherwise the scheme's standard port is assumed.
     */
    ...
    /** Binary (1024-based) mebibytes, not decimal megabytes. */
    ...
    /** Priority order is image > pdf > json > csv > text; {@code ""} if none match. */
    ...
    /** SVG returns {@code image/png} (rasterized on preview), not {@code image/svg+xml}. */
    ...
    /** A token expiring today is already considered expired. */
    ...
    /**
     * Unreadable manifests summarize as empty rather than throwing -- a list page must not
     * fail to render because one row carries a malformed manifest.
     *
     * <p>Must match {@code deriveRootFolder} in {@code upload/zip-builder.js}, which is what
     * the browser used to name the archive, or a folder gets labelled a bundle on the page
     * showing its own folder name.
     */
